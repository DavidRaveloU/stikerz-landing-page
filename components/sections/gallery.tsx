  ...
  return (
    <section ref={containerRef} className="relative py-32 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4">
        {/* Section header — igual que antes */}
        <motion.div
          ...
          <motion.div style={{ x }} className="flex gap-6 justify-center">
            {content.gallery.screenshots.map((screenshot, index) => (
              <GalleryCard
                ...
                index={index}
                Icon={screenshotIcons[index]}
                color={screenshotColors[index]}
              />
            ))}
          </motion.div>
        </div>

        {/* ── MÓVIL: scroll horizontal táctil con snap ── */}
        <div className="md:hidden">
          ...
          <div className="flex gap-5 overflow-x-auto snap-x snap-mandatory scroll-smooth pb-4 px-2 -mx-2 scrollbar-hide">
            {content.gallery.screenshots.map((screenshot, index) => (
              <div
                key={screenshot}
          ...
          <div className="flex justify-center gap-1.5 mt-4">
            {content.gallery.screenshots.map((_, i) => (
...
// Tarjeta extraída para reutilizar en desktop y móvil
        ...
        <div className="absolute inset-2 rounded-[2rem] bg-background overflow-hidden">
          <div className="h-full flex flex-col">
            <div className="p-3 border-b border-border flex items-center justify-center">
              <div className="text-xs font-bold text-primary">Stikerz</div>
            </div>
            <div className="flex-1 p-3">
              <div
                className={`h-full rounded-xl bg-linear-to-br ${color} flex items-center justify-center relative overflow-hidden`}
              >
                <motion.div
